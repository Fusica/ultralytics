...
def sample_pairs(img_root, label_root, output_dir, samples_per_folder=500, min_interval=30):
    ...
    print(f"找到的子文件夹: {img_folders}")

    # 如果没有子文件夹，直接处理根目录
    if not img_folders:
        print("未找到子文件夹，直接处理根目录")
        img_files = sorted([f for f in os.listdir(img_root) if f.endswith((".jpg", ".png"))])
        if img_files:
            process_folder(
                "",
                Path(img_root),
    ...
    for folder in img_folders:
        ...
        while len(selected_indices) < n_samples and valid_indices:
            ...
            # 使用较小的间隔
            invalid_range = range(max(0, idx - effective_interval), min(len(img_files), idx + effective_interval + 1))
            ...
            if len(valid_indices) < (n_samples - len(selected_indices)) * 2:
                ...
                valid_indices = [i for i in range(len(img_files)) if i not in set(selected_indices)]
        ...
        for idx in selected_indices:
            ...
            # 构建新的文件名，只在有文件夹前缀时添加
            new_img_name = f"{folder}_{img_file}" if folder else img_file
            new_label_name = f"{folder}_{label_file}" if folder else label_file
...
def process_folder(
    ...
    """抽取逻辑到单独的函数，方便复用"""
    if len(img_files) < samples_per_folder:
        print(f"警告: {folder_prefix} 中的样本数量不足 {samples_per_folder}, 实际数量: {len(img_files)}")
        return
    ...
    for idx in selected_indices:
        ...
        # 构建新的文件名，只在有文件夹前缀时添加
        new_img_name = f"{folder_prefix}_{img_file}" if folder_prefix else img_file
        new_label_name = f"{folder_prefix}_{label_file}" if folder_prefix else label_file
    ...
    print(f"已处理 {folder_prefix or '根目录'}: 选择了 {len(selected_indices)} 个样本")


if __name__ == "__main__":
    # 设置路径 - 根据实际路径修改
    img_root = "/Users/max/Downloads/2024-12-22-16-32-29/images"
    label_root = "/Users/max/Downloads/2024-12-22-16-32-29/labels"
    output_dir = "/Users/max/Downloads/2024-12-22-16-32-29/sampled_dataset"
