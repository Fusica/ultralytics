#include <ros/ros.h>
#include <std_msgs/Float64MultiArray.h>
#include <geometry_msgs/Point.h>

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <errno.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <arpa/inet.h>
#include <unistd.h>

#define RECV_BUUF_SIZE 64
#define SERVER_PORT 37260
#define SERVER_IP "192.168.1.25"

const int YAW_MIN = -135;  // 最小偏航角度
const int YAW_MAX = 135;   // 最大偏航角度
const int PITCH_MIN = -90; // 最小俯仰角度
const int PITCH_MAX = 25;  // 最大俯仰角度

const uint16_t crc16_tab[256] =
    {0x0, 0x1021, 0x2042, 0x3063, 0x4084, 0x50a5, 0x60c6, 0x70e7,
     0x8108, 0x9129, 0xa14a, 0xb16b, 0xc18c, 0xd1ad, 0xe1ce, 0xf1ef,
     0x1231, 0x210, 0x3273, 0x2252, 0x52b5, 0x4294, 0x72f7, 0x62d6,
     0x9339, 0x8318, 0xb37b, 0xa35a, 0xd3bd, 0xc39c, 0xf3ff, 0xe3de,
     0x2462, 0x3443, 0x420, 0x1401, 0x64e6, 0x74c7, 0x44a4, 0x5485,
     0xa56a, 0xb54b, 0x8528, 0x9509, 0xe5ee, 0xf5cf, 0xc5ac, 0xd58d,
     0x3653, 0x2672, 0x1611, 0x630, 0x76d7, 0x66f6, 0x5695, 0x46b4,
     0xb75b, 0xa77a, 0x9719, 0x8738, 0xf7df, 0xe7fe, 0xd79d, 0xc7bc,
     0x48c4, 0x58e5, 0x6886, 0x78a7, 0x840, 0x1861, 0x2802, 0x3823,
     0xc9cc, 0xd9ed, 0xe98e, 0xf9af, 0x8948, 0x9969, 0xa90a, 0xb92b,
     0x5af5, 0x4ad4, 0x7ab7, 0x6a96, 0x1a71, 0xa50, 0x3a33, 0x2a12,
     0xdbfd, 0xcbdc, 0xfbbf, 0xeb9e, 0x9b79, 0x8b58, 0xbb3b, 0xab1a,
     0x6ca6, 0x7c87, 0x4ce4, 0x5cc5, 0x2c22, 0x3c03, 0xc60, 0x1c41,
     0xedae, 0xfd8f, 0xcdec, 0xddcd, 0xad2a, 0xbd0b, 0x8d68, 0x9d49,
     0x7e97, 0x6eb6, 0x5ed5, 0x4ef4, 0x3e13, 0x2e32, 0x1e51, 0xe70,
     0xff9f, 0xefbe, 0xdfdd, 0xcffc, 0xbf1b, 0xaf3a, 0x9f59, 0x8f78,
     0x9188, 0x81a9, 0xb1ca, 0xa1eb, 0xd10c, 0xc12d, 0xf14e, 0xe16f,
     0x1080, 0xa1, 0x30c2, 0x20e3, 0x5004, 0x4025, 0x7046, 0x6067,
     0x83b9, 0x9398, 0xa3fb, 0xb3da, 0xc33d, 0xd31c, 0xe37f, 0xf35e,
     0x2b1, 0x1290, 0x22f3, 0x32d2, 0x4235, 0x5214, 0x6277, 0x7256,
     0xb5ea, 0xa5cb, 0x95a8, 0x8589, 0xf56e, 0xe54f, 0xd52c, 0xc50d,
     0x34e2, 0x24c3, 0x14a0, 0x481, 0x7466, 0x6447, 0x5424, 0x4405,
     0xa7db, 0xb7fa, 0x8799, 0x97b8, 0xe75f, 0xf77e, 0xc71d, 0xd73c,
     0x26d3, 0x36f2, 0x691, 0x16b0, 0x6657, 0x7676, 0x4615, 0x5634,
     0xd94c, 0xc96d, 0xf90e, 0xe92f, 0x99c8, 0x89e9, 0xb98a, 0xa9ab,
     0x5844, 0x4865, 0x7806, 0x6827, 0x18c0, 0x8e1, 0x3882, 0x28a3,
     0xcb7d, 0xdb5c, 0xeb3f, 0xfb1e, 0x8bf9, 0x9bd8, 0xabbb, 0xbb9a,
     0x4a75, 0x5a54, 0x6a37, 0x7a16, 0xaf1, 0x1ad0, 0x2ab3, 0x3a92,
     0xfd2e, 0xed0f, 0xdd6c, 0xcd4d, 0xbdaa, 0xad8b, 0x9de8, 0x8dc9,
     0x7c26, 0x6c07, 0x5c64, 0x4c45, 0x3ca2, 0x2c83, 0x1ce0, 0xcc1,
     0xef1f, 0xff3e, 0xcf5d, 0xdf7c, 0xaf9b, 0xbfba, 0x8fd9, 0x9ff8,
     0x6e17, 0x7e36, 0x4e55, 0x5e74, 0x2e93, 0x3eb2, 0xed1, 0x1ef0};

// CRC16计算函数
uint16_t CRC16_cal(uint8_t *ptr, uint32_t len, uint16_t crc_init)
{
    uint16_t crc = crc_init;
    while (len-- != 0)
    {
        uint8_t temp = (crc >> 8) & 0xFF;
        crc = (crc << 8) ^ crc16_tab[*ptr++ ^ temp];
    }
    return crc;
}

bool parse_attitude_data(unsigned char *data, int len, double &roll, double &pitch, double &yaw)
{
    if (len < 22)
    {
        ROS_ERROR("Data length too short to parse.");
        return false;
    }

    // 只解析 RPY 数据
    int16_t yaw_raw = (int16_t)(data[8] | (data[9] << 8));
    int16_t pitch_raw = (int16_t)(data[10] | (data[11] << 8));
    int16_t roll_raw = (int16_t)(data[12] | (data[13] << 8));

    // 转换为实际角度值
    roll = roll_raw / 10.0;
    pitch = pitch_raw / 10.0;
    yaw = yaw_raw / 10.0;

    return true;
}

class SiyiController {
private:
    ros::NodeHandle nh_;
    ros::Publisher rpy_pub_;
    ros::Subscriber target_sub_;
    int sockfd_;
    struct sockaddr_in server_addr_;
    
    // 控制相关的常量
    const float DEADZONE_RATIO = 0.1f;
    const float MAX_ADJUST_RATE = 1.0f;
    int image_center_x_ = 960;
    int image_center_y_ = 540;

    // 添加当前云台姿态记录
    double current_yaw_ = 0.0;
    double current_pitch_ = 0.0;
    bool initialized_ = false;  // 添加初始化标志
    const float INIT_TOLERANCE = 2.0f;  // 初始化位置允许的误差范围（度）

public:
    SiyiController() : nh_("~") {
        // 创建RPY数据发布器
        rpy_pub_ = nh_.advertise<std_msgs::Float64MultiArray>("/siyi/rpy", 10);
        
        // 订阅目标位置
        target_sub_ = nh_.subscribe<geometry_msgs::Point>("/pixel_coordinates", 10, 
            boost::bind(&SiyiController::targetCallback, this, _1));

        // 初始化UDP socket
        if ((sockfd_ = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
            ROS_ERROR("Failed to create socket: %s", strerror(errno));
            return;
        }

        // 设置服务器地址
        memset(&server_addr_, 0, sizeof(server_addr_));
        server_addr_.sin_family = AF_INET;
        server_addr_.sin_addr.s_addr = inet_addr(SERVER_IP);
        server_addr_.sin_port = htons(SERVER_PORT);

        ROS_INFO("Siyi Controller initializing, moving to home position...");
    }

    ~SiyiController() {
        if (sockfd_ >= 0) {
            close(sockfd_);
        }
    }

    void targetCallback(const geometry_msgs::Point::ConstPtr &msg) {
        // 如果还没有初始化完成，不处理控制命令
        if (!initialized_) {
            return;
        }

        // 计算偏移量
        float offset_x = (float)(msg->x - image_center_x_) / image_center_x_;
        float offset_y = (float)(msg->y - image_center_y_) / image_center_y_;

        // 检查是否在死区内
        if (fabs(offset_x) <= DEADZONE_RATIO && fabs(offset_y) <= DEADZONE_RATIO) {
            return;
        }

        // 计算目标角度
        float target_yaw = -offset_x * YAW_MAX;
        float target_pitch = -offset_y * PITCH_MAX;

        // 计算需要调整的角度差值
        float yaw_diff = target_yaw - current_yaw_;
        float pitch_diff = target_pitch - current_pitch_;

        // 限制单次调整幅度
        float yaw_adjust = yaw_diff;
        float pitch_adjust = pitch_diff;
        if (fabs(yaw_diff) > MAX_ADJUST_RATE) {
            yaw_adjust = (yaw_diff > 0) ? MAX_ADJUST_RATE : -MAX_ADJUST_RATE;
        }
        if (fabs(pitch_diff) > MAX_ADJUST_RATE) {
            pitch_adjust = (pitch_diff > 0) ? MAX_ADJUST_RATE : -MAX_ADJUST_RATE;
        }

        // 计算新的目标姿态
        float new_yaw = current_yaw_ + yaw_adjust;
        float new_pitch = current_pitch_ + pitch_adjust;

        // 限制总角度范围
        new_yaw = std::max(float(YAW_MIN), std::min(float(YAW_MAX), new_yaw));
        new_pitch = std::max(float(PITCH_MIN), std::min(float(PITCH_MAX), new_pitch));

        // 发送控制命令
        sendControlCommand(int(new_yaw), int(new_pitch));

        // 更新当前姿态
        current_yaw_ = new_yaw;
        current_pitch_ = new_pitch;
    }

    void sendControlCommand(int yaw_adjust, int pitch_adjust) {
        int16_t yaw_adjust_scaled = yaw_adjust * 10;
        int16_t pitch_adjust_scaled = pitch_adjust * 10;

        unsigned char send_buf[14] = {
            0x55, 0x66,  // 帧头
            0x01, 0x04,  // 命令字
            0x00, 0x00, 0x00, 0x0E,  // 数据长度
            0x00, 0x00,  // yaw
            0x00, 0x00,  // pitch
            0x00, 0x00   // CRC16
        };

        send_buf[8] = yaw_adjust_scaled & 0xFF;
        send_buf[9] = (yaw_adjust_scaled >> 8) & 0xFF;
        send_buf[10] = pitch_adjust_scaled & 0xFF;
        send_buf[11] = (pitch_adjust_scaled >> 8) & 0xFF;

        uint16_t crc_result = CRC16_cal(send_buf, 12, 0);
        send_buf[12] = crc_result & 0xFF;
        send_buf[13] = (crc_result >> 8) & 0xFF;

        if (sendto(sockfd_, send_buf, sizeof(send_buf), 0,
                  (struct sockaddr *)&server_addr_, sizeof(server_addr_)) < 0) {
            ROS_ERROR("Failed to send control command: %s", strerror(errno));
        }
    }

    void spin() {
        ros::Rate loop_rate(50);  // 50Hz
        unsigned char send_buf[] = {0x55, 0x66, 0x01, 0x00, 0x00, 0x00, 0x00, 0x0d, 0xe8, 0x05};
        unsigned char recv_buf[RECV_BUUF_SIZE] = {0};
        socklen_t addr_len = sizeof(struct sockaddr_in);

        // 等待云台回到中心位置
        while (ros::ok() && !initialized_) {
            // 发送姿态请求
            if (sendto(sockfd_, send_buf, sizeof(send_buf), 0,
                      (struct sockaddr *)&server_addr_, addr_len) < 0) {
                ROS_ERROR("Failed to send request: %s", strerror(errno));
                continue;
            }

            // 接收姿态数据
            int recv_len = recvfrom(sockfd_, recv_buf, RECV_BUUF_SIZE, 0,
                                  (struct sockaddr *)&server_addr_, &addr_len);

            if (recv_len > 0) {
                double roll, pitch, yaw;
                if (parse_attitude_data(recv_buf, recv_len, roll, pitch, yaw)) {
                    // 发布当前姿态
                    std_msgs::Float64MultiArray rpy_msg;
                    rpy_msg.data = {roll, pitch, yaw};
                    rpy_pub_.publish(rpy_msg);

                    // 如果不在中心位置，发送回中命令
                    if (fabs(yaw) > INIT_TOLERANCE || fabs(pitch) > INIT_TOLERANCE) {
                        sendControlCommand(0, 0);  // 发送回中命令
                        ROS_INFO("Initializing: Current position - Yaw: %.1f, Pitch: %.1f", yaw, pitch);
                    } else {
                        // 已经到达中心位置
                        initialized_ = true;
                        ROS_INFO("Initialization complete, ready for control commands");
                    }
                }
            }

            ros::spinOnce();
            loop_rate.sleep();
        }

        // 正常运行循环
        while (ros::ok()) {
            // 发送姿态请求
            if (sendto(sockfd_, send_buf, sizeof(send_buf), 0,
                      (struct sockaddr *)&server_addr_, addr_len) < 0) {
                ROS_ERROR("Failed to send request: %s", strerror(errno));
                continue;
            }

            // 接收姿态数据
            int recv_len = recvfrom(sockfd_, recv_buf, RECV_BUUF_SIZE, 0,
                                  (struct sockaddr *)&server_addr_, &addr_len);

            if (recv_len > 0) {
                double roll, pitch, yaw;
                if (parse_attitude_data(recv_buf, recv_len, roll, pitch, yaw)) {
                    std_msgs::Float64MultiArray rpy_msg;
                    rpy_msg.data = {roll, pitch, yaw};
                    rpy_pub_.publish(rpy_msg);
                }
            }

            ros::spinOnce();
            loop_rate.sleep();
        }
    }

    // 在解析姿态数据时更新当前姿态
    bool parse_attitude_data(unsigned char *data, int len, double &roll, double &pitch, double &yaw) {
        if (len < 22)
        {
            ROS_ERROR("Data length too short to parse.");
            return false;
        }

        // 解析 RPY 数据
        int16_t yaw_raw = (int16_t)(data[8] | (data[9] << 8));
        int16_t pitch_raw = (int16_t)(data[10] | (data[11] << 8));
        int16_t roll_raw = (int16_t)(data[12] | (data[13] << 8));

        // 转换为实际角度值
        roll = roll_raw / 10.0;
        pitch = pitch_raw / 10.0;
        yaw = yaw_raw / 10.0;

        // 更新当前姿态
        current_yaw_ = yaw;
        current_pitch_ = pitch;

        return true;
    }
};

int main(int argc, char *argv[]) {
    ros::init(argc, argv, "siyi_controller");
    SiyiController controller;
    controller.spin();
    return 0;
}