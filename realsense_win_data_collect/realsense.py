...
class Camera(object):
    ...
    def __init__(self, width=640, height=480, fps=30):  # 图片格式可根据程序需要进行更改
        ...
        # Access the depth sensor to set additional options
        depth_sensor = self.profile.get_device().first_depth_sensor()
        depth_sensor.set_option(rs.option.emitter_enabled, 1)  # Enable the emitter
        depth_sensor.set_option(rs.option.laser_power, 360)  # Set laser power for improved depth sensing

        # Additional depth configuration options
        depth_sensor.set_option(rs.option.visual_preset, 1)  # High accuracy preset
        depth_sensor.set_option(rs.option.depth_units, 0.001)  # Set depth units to millimeters
